...
function getImageSource(image) {
  ...
  class ChatClassComponent extends React.Component {
    constructor(props) {
      super(props);
      const { route } = this.props;
      const { user, receiverUsername, receiverProfile } = route.params;
      this.state = {
        messages: [],
        input: '',
        receiverData: { username: receiverUsername || '', profile: receiverProfile || '', name: '', id: user?.id || '' },
        myProfile: { username: '', profile: '' },
        otherUserData: user,
      };
    ...
    componentDidUpdate(prevProps, prevState) {
      if (this.state.messages.length > 0 && this.flatListRef.current) {
        this.flatListRef.current.scrollToEnd({ animated: true });
      }
      const { user, receiverUsername, receiverProfile } = this.props.route.params;
      if (user?.id !== prevProps.route.params.user?.id ||
          receiverUsername !== prevProps.route.params.receiverUsername ||
          receiverProfile !== prevProps.route.params.receiverProfile) {
        this.fetchReceiverData();
        this.fetchReceiver();
        this.fetchOtherUser();
      }
    ...
    fetchReceiver = async () => {
      ...
      try {
        const userDoc = await getDoc(doc(db, "users", user.id));
        if (userDoc.exists()) {
          this.setState({ receiverData: userDoc.data() });
        }
      } catch (error) {
        console.error("Error fetching receiver data:", error);
      }
    ...
    fetchOtherUser = async () => {
      ...
      const userDoc = await getDoc(doc(db, 'users', user.id));
      if (userDoc.exists()) {
        this.setState({ otherUserData: { id: user.id, ...userDoc.data() } });
      }
    };
    ...
    setupMessagesListener = () => {
      ...
      this.unsubscribeMessages = onSnapshot(q, snapshot => {
        this.setState({ messages: snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) });
      });
    };
    ...
    handleSend = async () => {
      ...
      const { user, chatId } = this.props.route.params;
      const currentUser = getAuth().currentUser;
    ...
    render() {
      ...
      const { user, senderUsername, senderProfile, receiverUsername, receiverProfile } = route.params;
      const { messages, input, receiverData, myProfile, otherUserData } = this.state;
      const currentUser = getAuth().currentUser;
      ...
      return (
        <SafeAreaView style={styles.container}>
          ...
          <FlatList
            ...
            onContentSizeChange={() => this.flatListRef.current.scrollToEnd({ animated: false })}
          />
...
export default ChatClassComponent;
